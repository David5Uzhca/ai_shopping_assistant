...
def get_recent_chat_history(user_id: str, limit: int = 50) -> List[Dict[str, Any]]:
    """
    Obtiene los últimos mensajes del historial para reconstruir contexto.
    Retorna lista ordenada por antigüedad (más viejo primero) para que el modelo entienda el flujo.
    """
    # Obtenemos los últimos N mensajes (ordenados por fecha desc)
    query = """
        SELECT role, content 
        FROM chat_history 
        WHERE user_id = %s 
...
def clear_chat_history(user_id: str):
    """
    Borra el historial de un usuario (opcional, para reiniciar memoria).
    """
